class Meetings::Meeting < ApplicationRecord
  ...
  def pretty_deployment_assigned_to
    attendees.map{|attendee| attendee.resourceable.full_name}.join(', ')
  end
  ...
  def emails_of_attendees
    self.attendees.map{ |attendee| attendee.resourceable.email }
  end
