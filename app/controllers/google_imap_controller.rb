class GoogleImapController < ApplicationController
  ...
  def callback
    if params[:code].present?
      ...
      current_user.update(google_code: params[:code])
      auth_client.fetch_access_token!
      uri = URI("https://www.googleapis.com/gmail/v1/users/me/profile?access_token=#{auth_client.access_token}")
      response =  JSON.parse(Net::HTTP.get(uri))
      current_user.update(google_access_token: auth_client.access_token, google_refresh_token: auth_client.refresh_token, google_email: response["emailAddress"], google_token_expired_at: auth_client.expires_at)
